...
Retrieval scoping: `/api/retrieve` and `/api/ask` accept optional `workspace_id`/`document_ids` that filter chunks by metadata tagged at index time (`document_ids` narrows a workspace; it never replaces it). Dense candidates are scope-filtered by over-fetching the whole index, lexical (BM25) candidates are filtered before the candidate cut, so both budgets are spent in scope. This is **local-backend only** — backends without `supports_inmemory_scope_filter` (qdrant/pgvector/faiss) raise `ScopeNotSupportedError` (HTTP 400) instead of silently returning under-filled results. Empty scope values are rejected (422) and `RAG_REQUIRE_WORKSPACE_SCOPE=1` makes retrieval without a workspace fail closed (400 `workspace_scope_required`) for multi-tenant hosts. `workspace_id`/`document_id` must not contain `/` (the derived identity would collide across workspaces).

One chunk owner: both `/api/index` (posted text, parsed by the native text parser) and `/api/index/file` go through the structure/token-aware chunker (`chunk/structured.py`); the recursive character chunker (`chunk/chunker.py`) remains the CLI knowledge-base pipeline. One retrieval owner: `/api/retrieve`, `/api/ask` and `/api/eval/run` share the hybrid path (`_hybrid_retrieve_response`) on enumerable backends. Parent grouping collapses field views of one entity (a fact sheet's fields); consecutive prose passages are distinct parents; the requested `top_k` is the parent budget. Every ranking change must be measured with the ControlRoom qualification benchmark (`python -m slimx_rag.eval.qualification`, frozen gate in `examples/controlroom_qualification/quality-gate.json`); see `docs/reviews/controlroom-qualification-2026-09-25.md`.

Per-document deletion and replacement are authoritative on enumerable backends: the bookkept chunk ids from `IndexState` are unioned with a sweep of every stored chunk tagged with the document's `doc_id` (`IndexBackend.delete_doc_detailed`), so a crash between `save()` and `commit_doc_state()` cannot leave stale content retrievable. Known failures (invalid state, unreadable receipt, dimension mismatch, embedder init) return the same structured reasons `/ready` reports (`_service_failure`), never bare 500s.
