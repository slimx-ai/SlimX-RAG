...
The CPU image built by `Dockerfile` is the only artifact qualified for ControlRoom. Every image
input is immutable (the BuildKit frontend and GitHub Actions runners are not part of the image):
...
| Python resolution | the committed `uv.lock`, checked against `pyproject.toml` (`uv lock --check`) and installed with `uv sync --frozen` (a build fails if the lock cannot be honoured); torch is the CPU wheel from `download.pytorch.org/whl/cpu` (`[tool.uv.sources]`), pinned by `constraint-dependencies` |
...
with SBOM and provenance attestations (BuildKit plus a GitHub build-provenance attestation on the pushed digest) and prints the registry digest; the `release` path carries BuildKit SBOM/provenance only. The digest, not the tag,
